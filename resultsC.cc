...
resultsC::resultsC(bool dm, bool di, bool du, bool dt) :
  ...
  otherTransport("OtherTransport"), synCount(0), finCount(0),
...
std::ostream &operator<<(std::ostream &os, const resultsC &c) {
    os << "ethernet: " << std::endl << c.ethernet << std::endl
       << "otherLink: " << std::endl  << c.otherLink << std::endl
    ...
    if (c.displayMacs) {
        ...
        for(auto I = c.srcMacSet.begin(); I != c.srcMacSet.end(); I++)
         os << "\t" << ether_ntoa((const struct ether_addr *)&((*I)[0])) << std::endl;
    ...
    if (c.displayUDP) {
        ...
        os << "\nUnique UDP Destination Ports" << std::endl;
    ...
    if (c.displayTCP) {
        ...
        os << "\nUnique TCP Destination Ports" << std::endl;
